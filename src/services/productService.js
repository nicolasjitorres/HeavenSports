...
const productService = {
  addToCart: async function (data, idProd, idUser) {
    ...
    if (productoTalle.stock >= data.cantidad_producto) {
      ...
      if (carritoProductoTalle) {
        console.log(data);
        let newCantidad =
          carritoProductoTalle.cantidad_producto +
          parseInt(data.cantidad_producto);
        ...
        const newCarritoProductoTalle = new CarritoProductoTalle(
          carrito.id,
          productoTalle.id,
  ...
  saveProduct: async function (data, files) {
    ...
    const { id } = await db.Producto.create(newProducto);
...
function ProductoCategoria(id_producto, id_categoria) {
  (this.id_producto = id_producto), (this.id_categoria = id_categoria);
}
...
function ProductoImagen(id_producto, id_imagen) {
  (this.id_producto = id_producto), (this.id_imagen = id_imagen);
}

// Constructor de CarritoProductoTalle
function CarritoProductoTalle(
  id_carrito,
  id_producto_talle,
  cantidad_producto
) {
